...
def main(
    ...
    print(run_configuration.get("profiles"))
    shutil.copy(run_configuration.get("profiles"), "data/profiles.json")
    ...
    if policy_model == "gpt":
        ...
        dialog4ref_queue = [
            deque(maxlen=7) for _ in range(env_config["n_agents"])
        ]
    ...
    if policy_model == "gpt":
        policy_model_save = (
            f"{policy_model}-{dialog_len}-noperception-reflection-1"
        )
    policy_model_save = f'{policy_model_save}-{num_agents}agents-{episode_length}months-{case}-{world_start_time.strftime("%Y.%m")}'
    print(f"{save_path}data/{policy_model_save}")
    ...
    for epi in range(env.episode_length):
        ...
        if (epi + 1) % 1 == 0 or epi + 1 == env.episode_length:
            ...
            if policy_model == "gpt":
                with open(
                    f"{save_path}data/{policy_model_save}/dialog_{epi+1}.pkl",
                    "wb",
                ) as f:
                    pkl.dump(dialog_queue, f)
                with open(
                    f"{save_path}data/{policy_model_save}/dialog4ref_{epi+1}.pkl",
                    "wb",
                ) as f:
                    pkl.dump(dialog4ref_queue, f)
            with open(
                f"{save_path}data/{policy_model_save}/dense_log_{epi+1}.pkl",
                "wb",
            ) as f:
