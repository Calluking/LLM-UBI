...
class BaseAgent:
    ...
    def set_component_action(self, component_name, action):
        ...
        if self._multi_action_dict[component_name]:
            ...
            self.action[component_name] = float(action)
        ...
        if self.multi_action_mode:
            ...
            if len(actions) == 1:
                ...
                for action_name, action in zip(self._action_names, actions):
                    ...
                    self.set_component_action(action_name, float(action))
...
Example:
    from ai_economist_ubi.foundation.base.base_agent import BaseAgent, agent_registry
