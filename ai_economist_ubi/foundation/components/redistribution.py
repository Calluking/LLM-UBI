...
from ai_economist_ubi.foundation.base.base_component import (
    BaseComponent,
    component_registry,
)
from ai_economist_ubi.foundation.components.utils import (
    annealed_tax_limit,
    annealed_tax_mask,
)
...
class PeriodicBracketTax(BaseComponent):
    ...
    def __init__(
        ...
        **base_component_kwargs
        ...
        assert self.bracket_spacing in ["linear", "log", "us-federal", "finland2017", "kenya2018", "us2020", "namibia2008"]
        ...
        elif self.bracket_spacing == "finland2017":
            self.bracket_cutoffs = (np.array([0, 20500, 30500, 50400, 88200, 150000])/12)
            self.n_brackets = len(self.bracket_cutoffs)
            self.top_bracket_cutoff = float(self.bracket_cutoffs[-1])
        elif self.bracket_spacing == "kenya2018":
            self.bracket_cutoffs = (np.array([0, 134164, 260567, 386970, 513373])/12*0.0077)
            self.bracket_cutoffs = np.array([int(i) for i in self.bracket_cutoffs])
            self.n_brackets = len(self.bracket_cutoffs)
            self.top_bracket_cutoff = float(self.bracket_cutoffs[-1])
        elif self.bracket_spacing == "us2020":
            self.bracket_cutoffs = (np.array([0, 9875, 40125, 85525, 163300, 207350, 518400])/12)
            self.n_brackets = len(self.bracket_cutoffs)
            self.top_bracket_cutoff = float(self.bracket_cutoffs[-1])
        elif self.bracket_spacing == "namibia2008":
            self.bracket_cutoffs = (np.array([0, 36000, 40000, 80000, 200000])/12*0.056)
            self.bracket_cutoffs = np.array([int(i) for i in self.bracket_cutoffs])
            self.n_brackets = len(self.bracket_cutoffs)
            self.top_bracket_cutoff = float(self.bracket_cutoffs[-1])
        else:
        ...
        if self.tax_model == "finland2017":
            assert self.bracket_spacing == "finland2017"
        
        if self.tax_model == "kenya2018":
            assert self.bracket_spacing == "kenya2018"  
        
        if self.tax_model == "us2020":
            assert self.bracket_spacing == "us2020" 
        
        if self.tax_model == "namibia2008":
            assert self.bracket_spacing == "namibia2008"
    ...
    @property
    def finland_2017_scaled(self):
        """
        https://taxsummaries.pwc.com/Finland/Individual/Taxes-on-personal-income
        """
        return [0.1264, 0.19, 0.3025, 0.34, 0.42, 0.44]

    @finland_2017_scaled.setter
    def finland_2017_scaled(self, bracket_rates):
        return bracket_rates

    @property
    def kenya_2018_scaled(self):
        """
        https://ke.icalculator.com/income-tax-rates/2018.html
        """
        return [0.1, 0.15, 0.20, 0.25, 0.3]

    @kenya_2018_scaled.setter
    def kenya_2018_scaled(self, bracket_rates):
        return bracket_rates
    
    @property
    def us_2018_scaled(self):
        """
        https://taxfoundation.org/data/all/federal/2020-tax-brackets/
        """
        return [0.1, 0.12, 0.22, 0.24, 0.32, 0.35, 0.37]

    @us_2018_scaled.setter
    def us_2018_scaled(self, bracket_rates):
        return bracket_rates
    
    @property
    def namibia_2008_scaled(self):
        """
        https://taxfoundation.org/data/all/federal/2020-tax-brackets/
        """
        return [0, 0.175, 0.295, 0.345, 0.35]

    @namibia_2008_scaled.setter
    def namibia_2008_scaled(self, bracket_rates):
        return bracket_rates
    # ------- fixed-bracket-rates
    @property
    ...
    def curr_marginal_rates(self):
        ...
        elif self.tax_model == "finland_2017_scaled":
            marginal_tax_bracket_rates = np.minimum(
                np.array(self.finland_2017_scaled), self.curr_rate_max
            )
        elif self.tax_model == "kenya_2018_scaled":
            marginal_tax_bracket_rates = np.minimum(
                np.array(self.kenya_2018_scaled), self.curr_rate_max
            )
        elif self.tax_model == "us_2020_scaled":
            marginal_tax_bracket_rates = np.minimum(
                np.array(self.us_2018_scaled), self.curr_rate_max
            )
        elif self.tax_model == "namibia_2008_scaled":
            marginal_tax_bracket_rates = np.minimum(
                np.array(self.namibia_2008_scaled), self.curr_rate_max
            )
        else:
            raise NotImplementedError
