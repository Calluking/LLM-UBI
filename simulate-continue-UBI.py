...
def gpt_actions(
    ...
    for idx in range(env.num_agents):
        ...
        problem_prompt = f"""
                    You're {name}, a {age}-year-old individual living in {city}. As with all people in {case}, a portion of your monthly income is taxed by the government. This taxation system is tiered, income is taxed cumulatively within defined brackets, combined with a social service policy: after collection, the government uses the tax revenue to provide social services to all citizens.
                    Now it's {current_time}.
                """
        if job == "Unemployment":
            job_prompt = f"""
                        In the previous month, you became unemployed and had no income. Now, you are invited to work as a(an) {offer} with hourly salary of ${skill:.2f}. You can work up to a maximum of {max_l} hours per month.
                    """
        else:
            if skill >= states[-1][str(idx)]["skill"]:
                job_prompt = f"""
                            In the previous month, you worked as a(an) {job}. If you continue working this month, your expected salary will be ${skill:.2f} per hour, which is increased compared to the last month due to the inflation of labor market.
                        """
            else:
                job_prompt = f"""
                            In the previous month, you worked as a(an) {job}. If you continue working this month, your expected salary will be ${skill:.2f} per hour, which is decreased compared to the last month due to the deflation of labor market.
                        """
        ...
        if (
            ...
            tax_prompt = f"""Your tax deduction amounted to ${tax_paid:.2f}. The government uses the taxes recieved to provide social services. The strength of the social service provided last month was ${lump_sum:.2f} per capita.
        ...
        obs_prompt = f"""
                        ...
                        Your goal is to maximize your utility by deciding how much to work and how much to consume."""
        obs_prompt += f"""
                        You are experiencing a universal basic income policy, receiving a monthly stipend of ${ubi}."""
        if description:
            obs_prompt += description
        obs_prompt += f"""
                        Your utility is determined by your consumption, income, saving, social service recieved and leisure time.
                        You will spend the time you do not work on leisure activities.
                        With all these factors in play, and considering aspects like your living costs, any future aspirations, and the broader economic trends, what proportion of your spare time (a total of 168 hours a month) would you spend working? Furthermore, how would you plan your expenditures on essential goods, keeping in mind good price?
                        Please share your decisions in a JSON format. The format should have two keys: 'work' (a value between 0 and 1 with intervals of 0.02, indicating the proportion of the 168 hours you spend to work) and 'consumption' (a value between 0 and 1 with intervals of 0.02, indicating the proportion of all your savings and income you intend to spend on essential goods).
                        Please ensure that the response is in JSON format, with 2 keys: 'work' and 'consumption'.
                    """
    ...
    if (env.world.timestep + 1) % 3 == 0:
        ...
        results, cost = get_multiple_completion_non_json(
...
def complex_actions(env, obs, beta=0.1, gamma=0.1, h=1):
    ...
    def consumption_cats(
        price, wealth, curr_income, last_income, interest_rate
    ):
        ...
        c = 1 + (d - h1 * g) / (1 + g + 1e-8)
        c = min(
            max(c * curr_income / (wealth + curr_income + 1e-8) // 0.02, 0), 50
        )
        return c

    def work_income_wealth(
        price, wealth, curr_income, last_income, expected_income, interest_rate
    ):
        return int(
            np.random.uniform()
            < (curr_income / (wealth * (1 + interest_rate) + 1e-8)) ** gamma
        )
    ...
    for idx in range(env.num_agents):
        ...
        if "consumption_fun_idx" not in this_agent.endogenous:
            this_agent.endogenous["consumption_fun_idx"] = np.random.choice(
                range(len(consumption_funs))
            )
        if "work_fun_idx" not in this_agent.endogenous:
            this_agent.endogenous["work_fun_idx"] = np.random.choice(
                range(len(work_funs))
            )
        ...
        consumption_fun = consumption_funs[
            this_agent.endogenous["consumption_fun_idx"]
        ]
        c = consumption_fun(
            price, wealth, curr_income, last_income, interest_rate
        )
...
def main(
    ...
    case="finland",
    duration=24,
    data_path="data/gpt-3-noperception-reflection-1-50agents-200months-finland-2000.05",
):
    print("starting")
    with open(f"config_{case}.yaml", "r") as f:
        run_configuration = yaml.safe_load(f)
    world_start_time = datetime.strptime(
        run_configuration.get("world_start_time"), "%Y.%m"
    )
    world_start_time = world_start_time - relativedelta(months=episode_length)
    ubi = run_configuration.get("ubi")
    run_configuration.pop("ubi")
    description = run_configuration.get("description")
    run_configuration.pop("description")

    actions = pkl.load(open(f"{data_path}/actions_{episode_length}.pkl", "rb"))
    dialog_queue = pkl.load(
        open(f"{data_path}/dialog_{episode_length}.pkl", "rb")
    )
    dialog4ref_queue = pkl.load(
        open(f"{data_path}/dialog4ref_{episode_length}.pkl", "rb")
    )
    obs = pkl.load(open(f"{data_path}/obs_{episode_length}.pkl", "rb"))
    env = pkl.load(open(f"{data_path}/env_{episode_length}.pkl", "rb"))
    env.components[1].ubi = ubi

    policy_model_save = f"{data_path}_continue_duration{duration}_ubi{ubi}"
    if not os.path.exists(policy_model_save):
        os.makedirs(policy_model_save)
    save_path = "./"
    gpt_error = 0
    total_cost = 0
    t = time()
    for epi in range(episode_length, episode_length + duration):
        if policy_model == "gpt":
            actions, gpt_error, total_cost = gpt_actions(
                ...
                dialog_queue,
                dialog4ref_queue,
                f"{save_path}{policy_model_save}/dialogs",
                gpt_error,
                total_cost,
                case,
                ubi,
        ...
        if (epi + 1) % 1 == 0 or epi + 1 == env.episode_length:
            with open(
                f"{save_path}{policy_model_save}/actions_{epi+1}.pkl", "wb"
            ) as f:
                pkl.dump(actions, f)
            with open(
                f"{save_path}{policy_model_save}/obs_{epi+1}.pkl", "wb"
            ) as f:
                pkl.dump(obs, f)
            with open(
                f"{save_path}{policy_model_save}/env_{epi+1}.pkl", "wb"
            ) as f:
                pkl.dump(env, f)
            if policy_model == "gpt":
                with open(
                    f"{save_path}{policy_model_save}/dialog_{epi+1}.pkl", "wb"
                ) as f:
                    pkl.dump(dialog_queue, f)
                with open(
                    f"{save_path}{policy_model_save}/dialog4ref_{epi+1}.pkl",
                    "wb",
                ) as f:
                    pkl.dump(dialog4ref_queue, f)
            with open(
                f"{save_path}{policy_model_save}/dense_log_{epi+1}.pkl", "wb"
            ) as f:
                pkl.dump(env.dense_log, f)

    with open(f"{save_path}{policy_model_save}/dense_log.pkl", "wb") as f:
        pkl.dump(env.dense_log, f)
